...
public class FileStagingService {
    ...
    private final FileStagingConfiguration configuration;

    public FileStagingService(FileStagingConfiguration configuration) {
        this.configuration = configuration;
    }

    public StagedUploadFile stage(MultipartFile file, String displayName, String tempFilePrefix, String logName) {
        long startedAt = System.nanoTime();
        validateFile(file);
        String fileName = validatedFileName(fileName(file, displayName));
        Path path = tempFile(fileName, tempFilePrefix);
        try {
            StagedUploadFile stagedFile = stageToPath(file, fileName, path);
            if (LOGGER.isInfoEnabled()) {
                LOGGER.info("{}文件暂存完成 文件名={} 文件大小={} 文件摘要前缀={} 耗时毫秒={}",
                        logName,
    ...
    private StagedUploadFile stageToPath(MultipartFile file, String fileName, Path path) {
        MessageDigest digest = sha256Digest();
        long size = copy(file, path, digest);
        validateSize(size);
        return new StagedUploadFile(path, fileName, size, hex(digest.digest()));
    }
    ...
    private long copy(MultipartFile file, Path path, MessageDigest digest) {
        try (InputStream inputStream = file.getInputStream();
            ...
            throw new YundocException(YundocErrorCode.INTERNAL_ERROR, "Failed to stage upload file", ex);
        }
    }
    ...
    private String normalizeFileName(String fileName) {
        ...
        return requireFileName(Paths.get(trimmed).getFileName()).toString();
    }

    private Path requireFileName(Path fileName) {
        if (fileName == null) {
            throw validationFailed();
        }
        return fileName;
    ...
    private void validateFileName(String fileName) {
        if (fileName.length() > configuration.getMaxFileNameLength()) {
            throw validationFailed();
        }
    ...
    private void validateExtension(String fileName) {
        ...
        if (!configuration.getAllowedExtensions().contains(extension(fileName))) {
            throw validationFailed();
        }
    }

    private boolean hasExtensionAllowlist() {
        return configuration.getAllowedExtensions() != null && !configuration.getAllowedExtensions().isEmpty();
    }
    ...
    private Path tempFile(String fileName, String tempFilePrefix) {
        try {
            return Files.createTempFile(tempDirectory(), tempFilePrefix, "-" + fileName);
        } catch (IOException ex) {
            throw new YundocException(YundocErrorCode.INTERNAL_ERROR, "Failed to create upload temp file", ex);
        }
    }

    private Path tempDirectory() {
        if (Texts.hasText(configuration.getTempDirectory())) {
            return Paths.get(configuration.getTempDirectory());
        }
        return Paths.get(System.getProperty("java.io.tmpdir"));
    }

    private void validateSize(long size) {
        if (size <= 0L || size > configuration.getMaxFileSizeBytes()) {
            throw validationFailed();
        }
    }
    ...
    private void deleteQuietly(Path path) {
        try {
            ...
            LOGGER.warn("临时上传文件删除失败 文件路径={}", path, ex);
        }
    }
