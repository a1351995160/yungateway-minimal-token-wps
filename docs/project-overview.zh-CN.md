...
业务系统需要使用 WPS 云文档接口，例如文件预览、用户文件列表、用户文件读写等。如果每个业务系统直接对接 WPS OpenAPI，会导致 WPS app 凭证分散、token 管理重复、权限边界不清晰、审计和限流难以统一。

本项目提供一个服务端接口网关，由网关统一保存和使用 WPS 凭证，业务系统只通过内部 JWT 调用网关暴露的对外接口。
...
src/main/java/com/wps/yundoc
  ...
  capability/       对外接口 Controller 和应用服务
